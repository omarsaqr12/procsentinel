...
impl FilterParser {
        ...
        for op in operators.iter() {
            if let Some(pos) = input.find(op) {
                ...
                if *op == ">" || *op == "<" || *op == ">=" || *op == "<=" {
                    ...
                    return match *op {
                        ">" => Ok(FilterExpression::FieldGreaterThan {
                            field: field,
                            value: num_value,
                        }),
                        "<" => Ok(FilterExpression::FieldLessThan {
                            field: field,
                            value: num_value,
                        }),
                        ">=" => Ok(FilterExpression::FieldGreaterEqual {
                            field: field,
                            value: num_value,
                        }),
                        "<=" => Ok(FilterExpression::FieldLessEqual {
                            field: field,
                            value: num_value,
                        }),
                        _ => unreachable!(),
                ...
                return match *op {
                    "==" => Ok(FilterExpression::FieldEquals {
                        field: field,
                        value: value.to_string(),
                    }),
                    "!=" => Ok(FilterExpression::FieldNotEquals {
                        field: field,
                        value: value.to_string(),
                    }),
    ...
    pub fn evaluate(&mut self, process: &ProcessInfo, expr: &FilterExpression) -> bool {
        match expr {
            ...
            FilterExpression::Not(expr) => !self.evaluate(process, expr),
        }
    }
